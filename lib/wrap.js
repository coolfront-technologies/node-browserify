...
Wrap.prototype.readFile = function (file) {
    if(process.env.DEBUG_BROWSERIFY){
...
Wrap.prototype.requireMultiple = function (startFiles) {
    ...
    if(process.env.DEBUG_BROWSERIFY){
      console.log("requireMultiple: "+ startFiles)
    }
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    if(process.env.DEBUG_BROWSERIFY){
      _requireStack.push(mfile)
    }

    if(process.env.DEBUG_BROWSERIFY){
      console.log("require: "+ mfile)
    }
    ...
    function done(){
      ...
      if(process.env.DEBUG_BROWSERIFY){
        let start = _requireStack.length - 20
        start = start < 0 ? 0 : start
        let str = ''
        for(let e = start; e < _requireStack.length; e++ ){
          str += _requireStack[e] + "->"
        }
        console.log(str)
        _requireStack.pop()
    ...
    if (typeof mfile === 'object') {
        let res = self.requireMultiple(mfile, opts);
        done()
        return res
    }

    if (self.ignoring[mfile]) return done();
    if (self.aliases[mfile]) return done();
    ...
    if (self.ignoring[opts.target]) return done();
    if (self.has(file, opts.target)) return done();
